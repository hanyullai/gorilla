...
def parallel_generate_on_device(device, model, input_ids, temperature = 0.95):
    with torch.no_grad():
        generated_outputs = model.generate(input_ids, max_length=50, num_return_sequences=1, do_sample=True, temperature=temperature)
...
def inference_moss(model, tokenizer, prompts):
    ...
    prompts = [query.format(prompt=prompt.strip()) for prompt in prompts]
